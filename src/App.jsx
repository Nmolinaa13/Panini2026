...
// Nomenclatura XXX## — posición 13 = foto equipo
  ...
  for (const [grp, teams] of Object.entries(GROUPS)) {
    for (const team of teams) {
      ...
      for (let j = 2; j <= 12; j++) {
        ...
        all.push({ id: `${team}${num}`, group: grp, team, label: `Jugador ${j - 1}`, type: "jugador" });
      }
      all.push({ id: `${team}13`, group: grp, team, label: "Foto equipo", type: "equipo" });
      for (let j = 14; j <= 20; j++) {
        const num = String(j).padStart(2, "0");
        all.push({ id: `${team}${num}`, group: grp, team, label: `Jugador ${j - 2}`, type: "jugador" });
      }
    }
  }
  for (let i = 1; i <= 14; i++) {
...
// Chip optimizado para móvil:
// - Sin lámina: botón simple → toca para agregar
// - Con lámina: chip verde + botón "−" visible al lado para quitar
function Chip({ sticker, count, onAdd, onRemove }) {
  ...
  const emoji = sticker.type === "escudo" ? "🛡"
    : sticker.type === "equipo" ? "📸"
    : sticker.type === "portada" ? "📖"
    : sticker.type === "coca" ? "🥤"
    : `J${sticker.label.replace("Jugador ", "")}`;

  if (!has) {
    return (
      <button onClick={onAdd} title={`${sticker.id} — ${sticker.label}`} style={{
        width: 44, height: 44, borderRadius: 10,
        border: "1.5px dashed #334155", background: "#0f172a", color: "#475569",
        fontSize: 9, fontWeight: 700, cursor: "pointer",
        display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
        gap: 1, fontFamily: "inherit",
      }}>
        <span style={{ fontSize: 8, opacity: 0.6, lineHeight: 1 }}>{num}</span>
        <span style={{ fontSize: 8, lineHeight: 1 }}>{emoji}</span>
      </button>
    );
  }
  ...
  return (
    <div style={{ display: "inline-flex", alignItems: "stretch", position: "relative" }}>
      {/* Botón principal — toca para agregar repetida */}
      <button onClick={onAdd} title={`${sticker.id} · tienes ${count} · toca para +1`} style={{
        ...
        <span style={{ fontSize: 8, opacity: 0.7, lineHeight: 1 }}>{num}</span>
        <span style={{ fontSize: 8, lineHeight: 1 }}>{emoji}</span>
      </button>
      ...
      <button onClick={onRemove} title="Quitar una" style={{
        ...
        display: "flex", alignItems: "center", justifyContent: "center",
        fontFamily: "inherit", lineHeight: 1, paddingBottom: 1,
      }}>−</button>
    </div>
  );
}
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#020617", fontFamily: "'DM Sans', sans-serif", color: "#fff" }}>
        ...
        <div style={{ maxWidth: 960, margin: "0 auto", display: "flex" }}>
          {tabs.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{
              ...
              padding: "11px 14px", fontSize: 12, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap",
      ...
      {!loading && (
        <main style={{ maxWidth: 960, margin: "0 auto", padding: "16px 12px 64px" }}>
            ...
            <div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
                <select value={groupFilter} onChange={e => setGroupFilter(e.target.value)} style={{
                  ...
                  color: "#fff", padding: "9px 10px", fontSize: 12,
                }}>
              ...
              <p style={{ fontSize: 11, color: "#475569", marginBottom: 12 }}>
                Toca el chip para <strong style={{ color: "#4ade80" }}>agregar</strong> · toca <strong style={{ color: "#4ade80" }}>−</strong> para quitar
              </p>
              ...
              {Object.entries(stickersByTeam).map(([team, stickers]) => {
                ...
                return (
                  <div key={team} style={{ marginBottom: 14, background: "#0f172a", borderRadius: 14, overflow: "hidden", border: "1px solid #1e293b" }}>
                    <div style={{
                      ...
                      <div style={{ flex: 1 }}>
                        <span style={{ fontFamily: "'Bebas Neue'", fontSize: 16, letterSpacing: 2 }}>{teamLabel}</span>
                    ...
                    <div key={team} style={{ marginBottom: 10, background: "#0f172a", borderRadius: 12, overflow: "hidden", border: "1px solid #1e293b" }}>
                      <div style={{ padding: "8px 12px", background: `${grpColor}18`, borderLeft: `3px solid ${grpColor}`, borderBottom: "1px solid #1e293b" }}>
                        <span style={{ fontFamily: "'Bebas Neue'", fontSize: 14, letterSpacing: 2 }}>
                          {TEAM_NAMES[team] || team} <span style={{ color: grpColor, fontSize: 11 }}>({stickers.length})</span>
                        </span>
                      </div>
                        ...
                        {stickers.map(s => (
                          <button key={s.id} onClick={() => { addSticker(s.id); notify(`✅ ${s.id} conseguida`, "ok"); }}
                            style={{
                              ...
                              color: "#94a3b8", padding: "5px 10px", fontSize: 11, cursor: "pointer", transition: "all 0.12s",
                            }}
                        ...
                        <div key={s.id} style={{
                          ...
                          <div style={{ display: "flex", gap: 4 }}>
                            <button onClick={() => { addSticker(s.id); notify(`✅ ${s.id} +1`, "ok"); }} style={{
                              ...
                              color: "#22c55e", width: 32, height: 32, cursor: "pointer", fontSize: 18,
                            }}>+</button>
                            <button onClick={() => { removeSticker(s.id); notify(`🗑 ${s.id} -1`, "info"); }} style={{
                              background: "#1e293b", border: "1px solid #334155", borderRadius: 6,
                              color: "#f43f5e", width: 32, height: 32, cursor: "pointer", fontSize: 18,
                            }}>−</button>
                          </div>
                        </div>
                      );
            ...
            <div style={{ maxWidth: 520 }}>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
                <input
                  ...
                  placeholder="COL01 COL05 FWC03 CC07..."
                  style={{
              ...
              <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 14, padding: 20, marginTop: 24 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, fontSize: 12 }}>
                  {[
                    ["COL01","Escudo Colombia"],
                    ["COL02–COL12","Jugadores 1–11"],
                    ["COL13","Foto de equipo"],
                    ["COL14–COL20","Jugadores 12–18"],
                    ["FWC00–FWC19","Especiales álbum"],
                    ["CC01–CC14","Coca-Cola"],
                  ].map(([code, desc]) => (
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                  {Object.entries(TEAM_NAMES).map(([code, name]) => (
                    <span key={code} title={name} style={{
                      background: "#1e293b", borderRadius: 6, padding: "3px 8px", fontSize: 11, color: "#94a3b8",
                    }}>
